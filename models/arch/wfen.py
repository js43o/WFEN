...
class GSA(nn.Module):
    ...
    def forward(self, x, prev_atns = None):
        ...
        if prev_atns is None:   # 이전 어텐션 값 없을 경우
            ...
            q, k, v = qkv.chunk(3, dim=1)   # 3배 늘어난 채널 수를 3개씩 나눔 (= 원래 채널 수)
...
class RSA(nn.Module):
    ...
    def forward(self, x, prev_atns = None):
        ...
        if prev_atns is None:
            ...
            out = rearrange(out, 'b (h w) (dh dw) c-> b (c) (h dh) (w dw)', h=h//wsize, w=w//wsize, dh=wsize, dw=wsize) # 윈도우 해제, 원래 공간 차원으로 통합
...
class HaarWavelet(nn.Module):
    def __init__(self, in_channels, grad=False):
        ...
        self.haar_weights = torch.cat([self.haar_weights] * self.in_channels, 0)    # 입력 채널 수만큼 복사(depth-wise conv 방식으로 따로따로 계산)
    ...
    def forward(self, x, rev=False):    # rev는 inverse transform 여부
        if not rev:
            ...
            out = F.conv2d(x, self.haar_weights, bias=None, stride=2, groups=self.in_channels) / 4.0    # <- 4.0은 그냥 평균 맞추는 스케일러
            
            out = out.reshape([x.shape[0], self.in_channels, 4, x.shape[2] // 2, x.shape[3] // 2])  # 4배로 늘어난 채널을 각각 분리하여 4개의 서브밴드 형성
            
            out = torch.transpose(out, 1, 2)    # (C, 4) -> (4, C), wavelet 종류별로 채널 묶음
            
            out = out.reshape([x.shape[0], self.in_channels * 4, x.shape[2] // 2, x.shape[3] // 2]) # conv 연산을 위해 4개를 다시 하나로 통합
            ...
            return F.conv_transpose2d(out, self.haar_weights, bias=None, stride=2, groups=self.in_channels)   # 채널 4개를 다시 하나로, 공간 해상도는 2배로 늘어남
...
class WFU(nn.Module):
    ...
    def forward(self, x_big, x_small):
        haar = self.HaarWavelet(x_big, rev=False)   # 인코더 feature (큰 거) 웨이블릿 변환
        ...
        a_ = self.channel_tranformation(torch.cat([x_small, a], dim=1)) # LF들끼리는 concat해서 enhancing
        out = self.InverseHaarWavelet(torch.cat([hvd, a_], dim=1), rev=True)    # 결과 HF랑 LF랑 다시 하나로 묶어서 역변환
...
class WFD(nn.Module):
    ...
    def __init__(self, dim_in, dim, need=False):
        ...
        self.need = need
        if need:
            self.first_conv = nn.Conv2d(dim_in, dim, kernel_size=1, padding=0)  # 첫 conv
...
class WFEN(nn.Module):
    ...
    def forward(self, input_img):
        ...
        x_trans = self.Transformer(x_trans0) # hw:16 c:160 => 중앙 블록. 여러 개 존재 가능
...
class WFEN_no_Wavelet(nn.Module):
    def __init__(
        ...
        self.Upsample1 = nn.Sequential(
            nn.Conv2d(min_ch*2*2, min_ch*2*2*2*2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.PixelShuffle(2)
        )
        self.Upsample2 = nn.Sequential(
            nn.Conv2d(min_ch*2*2, min_ch*2*2*2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.PixelShuffle(2)
        )
        self.Upsample3 = nn.Sequential(
            nn.Conv2d(min_ch*2, min_ch*2*2, kernel_size=3, stride=1, padding=1, bias=False),
            nn.PixelShuffle(2)
        )
    ...
    def forward(self, input_img):
        ...
        ############ decoder ############
        x_up1 = self.Upsample1(x3 + x_trans) # hw:32 c:160   (채널 수 그대로, 해상도만 2배)
