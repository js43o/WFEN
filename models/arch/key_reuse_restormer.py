...
class KeyReuseAttention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        super(KeyReuseAttention, self).__init__()
        self.num_heads = num_heads
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))

        self.qv = nn.Conv2d(dim, dim * 2, kernel_size=1, bias=bias)
        self.qv_dwconv = nn.Conv2d(
            dim * 2,
            dim * 2,
            kernel_size=3,
            stride=1,
    ...
    def forward(self, x, key):
        assert key is not None, "key must be provided for KeyReuseAttention"

        b, c, h, w = x.shape

        qv = self.qv_dwconv(self.qv(x))
        q, v = qv.chunk(2, dim=1)  # 채널 수를 2배로 확장 후 q/v로 분할
        ...
        # (c, hw) @ (hw, c) => (c, c) attention map
        attn = (q @ key.transpose(-2, -1)) * self.temperature
        attn = attn.softmax(dim=-1)
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        if reuse_key:
            self.attn = KeyReuseAttention(dim, num_heads, bias)
        else:
    ...
    def forward(self, x, key=None):
        if key is None:
            attn_val, k = self.attn(self.norm1(x))
            x = x + attn_val
            x = x + self.ffn(self.norm2(x))
            return x, k
        else:
            x = x + self.attn(self.norm1(x), key)
            x = x + self.ffn(self.norm2(x))
            return x
...
##########################################################################
## query 재사용 Restormer
class KeyReuseRestormer(nn.Module):
    def __init__(
        self,
        inp_channels=3,
        out_channels=3,
        ...
        print("🍊 Key-Reuse Restormer architecture.")
        super(KeyReuseRestormer, self).__init__()
    ...
    def forward(self, inp_img):
        ...
        for i, block in enumerate(self.decoder_level3):
            out_dec_level3 = block(out_dec_level3, k3[-(i + 1)])
        ...
        for i, block in enumerate(self.decoder_level2):
            out_dec_level2 = block(out_dec_level2, k2[-(i + 1)])
        ...
        for i, block in enumerate(self.decoder_level1):
            out_dec_level1 = block(out_dec_level1, k1[-(i + 1)])

        for block in self.refinement:
            out_dec_level1, _k = block(out_dec_level1)
