...
class MapReuseAttention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        super(MapReuseAttention, self).__init__()
        self.num_heads = num_heads
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
    ...
    def forward(self, x, attn_map):
        assert attn_map is not None, "attention map must be provided for MapReuseAttention"
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        if reuse_map:
            self.attn = MapReuseAttention(dim, num_heads, bias)
        else:
    ...
    def forward(self, x, map=None):
        if map is None:
            ...
            x = x + self.attn(self.norm1(x), map)
            x = x + self.ffn(self.norm2(x))
            return x
...
##########################################################################
## query 재사용 Restormer
class MapReuseRestormer(nn.Module):
    def __init__(
        self,
        inp_channels=3,
        out_channels=3,
        ...
        print("🍊 Map-Reuse Restormer architecture.")
        super(MapReuseRestormer, self).__init__()
    ...
    def forward(self, inp_img):
        ...
        for i, block in enumerate(self.decoder_level3):
            out_dec_level3 = block(out_dec_level3, m3[-(i + 1)])
        ...
        for i, block in enumerate(self.decoder_level2):
            out_dec_level2 = block(out_dec_level2, m2[-(i + 1)])
        ...
        for i, block in enumerate(self.decoder_level1):
            out_dec_level1 = block(out_dec_level1, m1[-(i + 1)])

        for block in self.refinement:
            out_dec_level1, _m = block(out_dec_level1)
