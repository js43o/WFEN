...
class ValueReuseAttention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        super(ValueReuseAttention, self).__init__()
        self.num_heads = num_heads
        self.temperature = nn.Parameter(torch.ones(num_heads, 1, 1))
    ...
    def forward(self, x, value):
        assert value is not None, "value must be provided for ValueReuseAttention"

        b, c, h, w = x.shape

        qk = self.qk_dwconv(self.qk(x))
        q, k = qk.chunk(2, dim=1)  # 채널 수를 2배로 확장 후 q/k로 분할
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        if reuse_value:
            self.attn = ValueReuseAttention(dim, num_heads, bias)
        else:
    ...
    def forward(self, x, value=None):
        if value is None:
            attn_val, v = self.attn(self.norm1(x))
            x = x + attn_val
            x = x + self.ffn(self.norm2(x))
            return x, v
        else:
            x = x + self.attn(self.norm1(x), value)
            x = x + self.ffn(self.norm2(x))
            return x
...
##########################################################################
## query 재사용 Restormer
class ValueReuseRestormer(nn.Module):
    def __init__(
        self,
        inp_channels=3,
        out_channels=3,
        ...
        print("🍊 Value-Reuse Restormer architecture.")
        super(ValueReuseRestormer, self).__init__()
    ...
    def forward(self, inp_img):
        ...
        for i, block in enumerate(self.decoder_level3):
            out_dec_level3 = block(out_dec_level3, v3[-(i + 1)])
        ...
        for i, block in enumerate(self.decoder_level2):
            out_dec_level2 = block(out_dec_level2, v2[-(i + 1)])
        ...
        for i, block in enumerate(self.decoder_level1):
            out_dec_level1 = block(out_dec_level1, v1[-(i + 1)])

        for block in self.refinement:
            out_dec_level1, _v = block(out_dec_level1)
