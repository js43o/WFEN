...
# 설정
# ──────────────────────────────────────────
HQ_PATH   = "results/blind_ffhq_custom-aligned_128/lfw_custom-aligned/hr"
PRED_PATH = "results/blind_ffhq_custom-aligned_128/lfw_custom-aligned/pred"
...
def extract_features(folder: str, filenames: list[str]) -> np.ndarray:
    feats = []
    for i in tqdm(range(0, len(filenames), BATCH), desc=f"  {os.path.basename(folder)}"):
...
best_acc = max(((scores[None] >= thresholds[:, None]).astype(int) == gt[None]).mean(axis=1))
